...
with col3:
    # **NEW** ask user for starting chainage of the KML
...
if not (up_srt and up_kml):
    st.info("Please upload both an SRT and your Full‐chainage KML.")
    st.stop()

# ──────────────────────────────────────────────────────────────────────────
# Helper functions (unchanged except we now expect offsets applied below)
# ──────────────────────────────────────────────────────────────────────────

def hav(p, q):
    R=6371.0088
    φ1,φ2=math.radians(p[0]),math.radians(q[0])
    dφ, dλ = math.radians(q[0]-p[0]), math.radians(q[1]-p[1])
...
def parse_srt(uploaded):
    ...
    while i<len(lines):
        if not lines[i].strip().isdigit():
            i+=1; continue
        idx=int(lines[i].strip()); rng=lines[i+1].strip(); j=i+2
...
def kml_from_srt(blocks, cumd, si, ei):
    xml=[kml_header()]
    # SRT route polyline
    xml.append("<Placemark><styleUrl>#srtStyle</styleUrl><name>SRT Route</name>"
               "<LineString><coordinates>\n")
    xml += [f"{b['lon']:.6f},{b['lat']:.6f},{b['alt']:.2f}\n" for b in blocks]
    xml.append("</coordinates></LineString></Placemark>\n")
    # START/END placemarks
    s,e=blocks[si],blocks[ei]
    xml.append(f"<Placemark><styleUrl>#srtStyle</styleUrl><name>START {cumd[si]:.3f} km</name>"
               f"<Point><coordinates>{s['lon']:.6f},{s['lat']:.6f},{s['alt']:.2f}</coordinates></Point></Placemark>\n")
    xml.append(f"<Placemark><styleUrl>#srtStyle</styleUrl><name>END {cumd[ei]:.3f} km</name>"
               f"<Point><coordinates>{e['lon']:.6f},{e['lat']:.6f},{e['alt']:.2f}</coordinates></Point></Placemark>\n")
    xml.append("</Document></kml>")
    return "".join(xml)

def srt_mapping(blocks, prefix, offset=0.0):
    cum = cumulative_dist([(b['lat'],b['lon']) for b in blocks], offset=offset)
    mapping={}
    labels=["01_Latitude","02_Longitude","03_Altitude","04_Timer","05_LatLonOutput","06_Chainage"]
    contents=[
    ...
    for lab, cont in zip(labels, contents):
...
# ──────────────────────────────────────────────────────────────────────────
# Steps 2–9: run once uploads exist
# ──────────────────────────────────────────────────────────────────────────
...
if st.session_state.proceed:
    ...
    cum_srt=cumulative_dist(pts)
    si, sk = project_to_line(pts[0], chain_coords, cum_chain)
    ei, ek = project_to_line(pts[-1],chain_coords,cum_chain)

    st.subheader("Step 4: SRT Stationing")
    st.write(f"• Start → **{sk:.3f} km**")
    st.write(f"• End   → **{ek:.3f} km**")
    st.write(f"• Total flight length: **{cum_srt[-1]*1000:.1f} m**")
    ...
    kml0 = kml_from_srt(blks, cumulative_dist(pts, offset=chain_offset)[si:ei+1], si, ei)
    st.download_button("Download initial_srt_route.kml", kml0,
                       "initial_srt_route.kml","application/vnd.google-earth.kml+xml")

    map0=srt_mapping(blks,"initial",offset=sk)
    b0=io.BytesIO(); z0=zipfile.ZipFile(b0,'w')
    for fn,txt in map0.items(): z0.writestr(fn,txt)
    z0.close(); b0.seek(0)
    st.download_button("Download initial SRT files ZIP",b0.read(),
