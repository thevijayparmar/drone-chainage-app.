...
def parse_srt(uploaded):
    ...
    while i < len(lines):
        ...
        if lat is not None and lon is not None:
            blocks.append({'idx':idx,'range':rng,
...
def kml_header():
    return (
        ...
        "<Style id='routeStyle'>\n"
        "  <LineStyle><color>ffff0000</color><width>10.0</width></LineStyle>\n"
        "</Style>\n"
    )

@st.cache_data
def kml_from_markers(markers):
    # Points—no line style
    header = (
        "<?xml version='1.0' encoding='UTF-8'?>\n"
        "<!-- Author: Vijay Parmar; Community: BGol Community of Advanced Surveying and GIS Professionals -->\n"
        "<kml xmlns='http://www.opengis.net/kml/2.2'><Document>\n"
    ...
    return header + body + "</Document></kml>"
...
def kml_from_srt(blocks, cumd, si, ei):
    xml = [kml_header()]
    xml.append("<Placemark><styleUrl>#routeStyle</styleUrl><name>SRT Route</name>"
    ...
    xml.append(
        ...
        f"<Point><coordinates>{s['lon']:.6f},{s['lat']:.6f},{s['alt']:.2f}</coordinates>"
        f"</Point></Placemark>\n"
    )
    xml.append(
        f"<Placemark><styleUrl>#routeStyle</styleUrl>"
        f"<name>END {cumd[ei]:.3f} km</name>"
        f"<Point><coordinates>{e['lon']:.6f},{e['lat']:.6f},{e['alt']:.2f}</coordinates>"
        f"</Point></Placemark>\n"
    )
...
def srt_mapping(blocks, prefix, offset=0.0):
    ...
    mapping = {}
    mapping[f"{prefix}_01_Latitude.srt"]     = [f"{b['lat']:.6f}" for b in blocks]
    mapping[f"{prefix}_02_Longitude.srt"]    = [f"{b['lon']:.6f}" for b in blocks]
    mapping[f"{prefix}_03_Altitude.srt"]     = [f"{b['alt']:.2f}" for b in blocks]
    mapping[f"{prefix}_04_Timer.srt"]        = [b['tim'] for b in blocks]
    mapping[f"{prefix}_05_LatLonOutput.srt"] = [f"{b['lat']:.6f}, {b['lon']:.6f}" for b in blocks]
    mapping[f"{prefix}_06_Chainage.srt"]     = [f"{d:.3f} km" for d in cum]
    # join into text
    for fn, lines in list(mapping.items()):
        content = "\n\n".join(
            f"{blocks[i]['idx']}\n{blocks[i]['range']}\n{lines[i]}"
...
if up_srt and up_kml:
    ...
    if st.button("▶ Generate & Download 50 m Markers KML"):
        ...
        st.download_button("Download markers_50m.kml", kmlm, "markers_50m.kml",
    ...
    if st.session_state.proceed:
        ...
        buf0 = io.BytesIO()
        z0 = zipfile.ZipFile(buf0, 'w')
        for fn,content in map0.items(): z0.writestr(fn, content)
        z0.close(); buf0.seek(0)
        st.download_button("Download initial SRT files ZIP",
                           buf0.read(), "initial_srt_files.zip",
        ...
        if up_bulk:
            ...
            for bf in up_bulk:
                ...
                mapping = srt_mapping(blks, name, offset=sk)
                for fn,content in mapping.items(): zall.writestr(fn,content)
            zall.close(); all_buf.seek(0)
            st.download_button("Download all bulk outputs ZIP",
                               all_buf.read(), "all_bulk_outputs.zip",
                               mime="application/zip")
