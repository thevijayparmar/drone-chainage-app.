...
def generate_markers(coords, cumd, interval_km=0.05):
    ...
    while d <= total:
        ...
        markers.append((A[0]+frac*(B[0]-A[0]), A[1]+frac*(B[1]-A[1]), d))
        d += interval_km
    return markers
...
def kml_from_markers(markers):
    ...
    for lat,lon,d in markers:
        lines.append(f"<Placemark><name>{d:.3f} km</name>"
...
def kml_from_srt(blocks, cumd, si, ei):
    ...
    lines.append(f"<Placemark><name>START {cumd[si]:.3f} km</name>"
                 f"<Point><coordinates>{s['lon']:.6f},{s['lat']:.6f},{s['alt']:.2f}</coordinates></Point></Placemark>")
    lines.append(f"<Placemark><name>END {cumd[ei]:.3f} km</name>"
...
if up_srt and up_kml:
    ...
    proceed_marker = st.session_state.get("proceed_marker", False)

    # ───────────────────────────────────────────────────────
    # 2. Generate markers + FLIP toggle + Proceed
    # ───────────────────────────────────────────────────────
    ...
    if st.button("▶ Generate & Download 50 m Markers KML"):
        kml_m = kml_from_markers(generate_markers(chain_coords, cum_chain))
        st.download_button("Download markers_50m.kml", kml_m, "markers_50m.kml",
                           mime="application/vnd.google-earth.kml+xml")

    if st.button("Proceed further"):
        st.session_state.proceed_marker = True
        proceed_marker = True

    # ───────────────────────────────────────────────────────
    # After Proceed → Steps 4–9
    # ───────────────────────────────────────────────────────
    if proceed_marker:
        # 4. Project initial SRT start/end
        ...
        st.subheader("Step 4–5: SRT Stationing")
        st.write(f"- **Start** at {start_km:.3f} km on full-chainage")
        st.write(f"- **End** at {end_km:.3f} km on full-chainage")
        st.write(f"- **Total flight length**: {cum0[-1]*1000:.1f} m")

        # 6. Visualize
        st.subheader("Step 6: Visualize in 3D")
        fig = go.Figure()
        ...
        st.download_button("Download initial_srt_route.kml", kml0,
                           "initial_srt_route.kml", mime="application/vnd.google-earth.kml+xml")
