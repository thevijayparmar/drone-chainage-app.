...
if up_srt and up_kml:
    ...
    # ──────────────────────────────────────────────────────────────────────
    # 2. Generate 50 m Markers + FLIP toggle + Proceed
    # ──────────────────────────────────────────────────────────────────────
    ...
    proceed = st.button("Proceed further")
    ...
    if proceed:
        ...
        st.write(f"• Start → **{start_km:.3f} km**")
        st.write(f"• End   → **{end_km:.3f} km**")
        ...
        # 7. Export initial SRT‐derived
        st.subheader("Step 7: Export Initial SRT‐Derived Files")
        kml0 = kml_from_srt(blocks0, [start_km + d for d in cum0], si0, ei0)
        st.download_button(
            "Download initial_srt_route.kml", kml0, "initial_srt_route.kml",
            mime="application/vnd.google-earth.kml+xml"
        )
        zip0 = srt_zip(blocks0, "initial", offset=start_km)
        st.download_button(
            "Download initial SRT files ZIP", zip0, "initial_srt_files.zip",
